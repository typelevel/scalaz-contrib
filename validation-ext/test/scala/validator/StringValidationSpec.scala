...
class StringValidationSpec extends Specification {
  ...
  "not blank validator" should {
    ...
    "succeed when the string is not blank" in {
      forall(List("1", "              1     ")) { s => failNotBlank(s) must beNone }
    }
  }
  ...
  "strLength validation" should {
    ...
    "succeed when correct" in {
      forall(List("", "1", "12", "123"))   { x => maxThree(x) must beNone }
      forall(List("123", "1234", "12345")) { x => minThree(x) must beNone }
      eqThree("123") must beNone
    }

    "fail when invalid" in {
      forall(List("1234", "12345")) { x => maxThree(x) must beSome(errorMessage) }
      forall(List("", "1", "12"))   { x => minThree(x) must beSome(errorMessage) }
      forall(List("", "1", "12", "1234", "12345")) { num => eqThree(num) must beSome(errorMessage) }
    }
  }
